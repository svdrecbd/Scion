...
workset-convert:
	python3 workers/ingestion/private_workset_derivative.py convert --workset "$(WORKSET)" $(if $(WORKSET_ASSET_ID),--asset-id "$(WORKSET_ASSET_ID)",--relative-path "$(WORKSET_ASSET)") $(if $(WORKSET_CHUNKS),--chunk-shape "$(WORKSET_CHUNKS)",)
