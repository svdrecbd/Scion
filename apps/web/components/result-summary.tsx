  ...
  return (
    <div className="result-summary-box" style={{ marginBottom: 24, padding: "16px", background: "var(--accent)", borderRadius: 0, border: "1px solid var(--border)" }}>
      <p style={{ margin: 0, lineHeight: 1.6, fontSize: "1.05rem" }}>
        {hasFilters ? "In this slice of the corpus, " : "Across the entire corpus, "}
        the dominant imaging approach is <strong>{topModality}</strong>. 
        Researchers most frequently segmented <strong>{topOrganelle}</strong> 
        {topMetric ? (
          <> and primarily reported <strong>{topMetric}</strong> metrics.</>
        ) : "."}
        {commonalities.top_organelle_pairs.length > 0 && (
          <> A significant portion of these studies specifically targeted <strong>{commonalities.top_organelle_pairs[0]}</strong> contacts.</>
        )}
      </p>
    </div>
  );
