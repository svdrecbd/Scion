...
export default async function AnalyticsPage({
  ...
  searchParams: { row?: string; col?: string };
}) {
  const rowDim = searchParams.row || "cell_type";
  const colDim = searchParams.col || "public_data_status";

  const data = await getAnalyticsCrossTab(rowDim, colDim, searchParams);
  const benchmarks = await getAnalyticsBenchmarks();
  const toolkit = await getToolkitMatrix(searchParams);
  const frontier = await getFrontierData(searchParams);
              ...
              {data.rows.map((row: string) => (
                <tr key={row}>
                  ...
                  {data.cols.map((col: string) => {
                    ...
                    // Build search URL for this cell
                    const params = new URLSearchParams();
                    params.set(rowDim === "modality_family" ? "family" : rowDim, row);
                    
                    // Handle special cases for col mappings
